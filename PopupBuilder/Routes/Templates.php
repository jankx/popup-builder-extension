	...
	private function get_cached_templates()
	{
		...
		if (file_exists($cache_file)) {
			...
			if(!empty($modified_time) && $modified_time->format('Y-m-d H:i:s') > $last_check_time) {
				...
				$templates = $this->fetch_and_cache_templates($cache_file);
			} else {
				$data = file_get_contents($cache_file);
				$templates = json_decode($data, true);
			}

			// Filter out pro templates, only keep free templates
			if (! empty($templates) && is_array($templates)) {
				$free_templates = array_filter($templates, function($template) {
					return isset($template['package']) && $template['package'] === 'free';
				});
				return array_values($free_templates); // Re-index array
			}
		}
